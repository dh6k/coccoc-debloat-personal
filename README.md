...
## Khác gì so với repo gốc

Bảng này so sánh fork hiện tại với repo gốc `bibicadotnet/coccoc-debloat`.

| Hạng mục | Repo gốc | Fork này |
| --- | --- | --- |
| Lệnh cài nhanh | `irm https://go.bibica.net/coccoc \| iex` | `irm https://coccoc.33166099.xyz \| iex` |
| Nguồn mã/tải thủ công | Trỏ về `bibicadotnet/coccoc-debloat` và nhánh/bản phát hành gốc. | Trỏ về fork `dh6k/coccoc-debloat-personal` và mô tả cách chạy từ repo local. |
| Script online | Không tách riêng trong phần hướng dẫn chính. | Có `install-coccoc-online.ps1`, tải `mv2.ps1` và `coccoc-debloat.reg` từ GitHub raw của fork. |
| Script local | Hướng dẫn chạy `./install-coccoc.ps1` sau khi tải source. | `install-coccoc.ps1` là bản local/offline, dùng `mv2.ps1`, `coccoc-restore.reg`, `coccoc-debloat.reg` cùng thư mục. |
| Manifest V2 | Không mô tả patch `browser.dll` trong phần hướng dẫn chính. | Có `mv2.ps1`, copy/tải vào thư mục version Chromium rồi patch trực tiếp `browser.dll`. |
| Shortcut mặc định | Mô tả việc tắt Split View/Side Panel qua `--disable-features=CocCocSplitView,SidePanel`. | Installer chính không thêm flag tắt Split View/Side Panel vào shortcut mặc định; profile riêng vẫn có mục cấu hình riêng bên dưới. |
| Dọn dẹp hậu cài đặt | Không liệt kê bước dọn file trong thư mục version. | Có bước xóa file installer thừa, `browser.dll.BAK`, các `.crx` và `.json` mặc định trong `Extensions`. |
| Bản x86 | Không tách riêng trong phần hướng dẫn chính. | Có ghi chú riêng cho `install-coccoc-x86.ps1` và các khác biệt của bản x86. |
| Tài liệu kỹ thuật | Thiên về hướng dẫn sử dụng và tùy chỉnh. | Mô tả thêm quy trình installer, khác nhau giữa online/local, policy registry, MV2 patch và cleanup. |
