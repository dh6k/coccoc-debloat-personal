...
Bảng này so với README gốc trong `D:\Downloads\README.md`.

| Hạng mục | README gốc | Fork này |
| --- | --- | --- |
...
| Script online | Không tách riêng trong README gốc. | Có `install-coccoc-online.ps1`, tải `mv2.ps1` và `coccoc-debloat.reg` từ GitHub raw của fork. |
...
| Manifest V2 | README gốc không mô tả patch `browser.dll`. | Có `mv2.ps1`, copy/tải vào thư mục version Chromium rồi patch trực tiếp `browser.dll`. |
| Shortcut mặc định | README gốc mô tả việc tắt Split View/Side Panel qua `--disable-features=CocCocSplitView,SidePanel`. | Installer chính không thêm flag tắt Split View/Side Panel vào shortcut mặc định; profile riêng vẫn có mục cấu hình riêng bên dưới. |
| Dọn dẹp hậu cài đặt | README gốc không liệt kê bước dọn file trong thư mục version. | Có bước xóa file installer thừa, `browser.dll.BAK`, các `.crx` và `.json` mặc định trong `Extensions`. |
| Bản x86 | README gốc không tách riêng trong phần chính. | Có ghi chú riêng cho `install-coccoc-x86.ps1` và các khác biệt của bản x86. |
| Tài liệu kỹ thuật | README gốc thiên về hướng dẫn sử dụng và tùy chỉnh. | README fork mô tả thêm quy trình installer, khác nhau giữa online/local, policy registry, MV2 patch và cleanup. |
